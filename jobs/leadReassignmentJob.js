 ...
 *  - Once every eligible agent has had a turn unaccepted → DON'T stop: reset the
 *    rotation and keep revolving (a fresh 15-min turn per agent) until someone
 *    Accepts or an admin manually assigns. Admins are pinged once per full round.
 *
...
const processDueLeads = async () => {
  ...
  for (const lead of dueLeads) {
    try {
      const { action, next } = await advancePendingLead(lead, { now });
      if (action === 'reassigned') {
        console.log(`[REASSIGN] Lead ${lead._id} reassigned to ${next}`);
      } else if (action === 'recycled') {
        console.log(`[REASSIGN] Lead ${lead._id} completed a full cycle unaccepted — revolving again (now ${next})`);
      }
    } catch (err) {
